...
use MetricPulse\Includes\Service\GoogleAnalyticsService;
...
class GoogleAnalyticsServiceTest extends TestCase {
	...
	public function test_is_demo_mode_default() {
		update_option( 'metricpulse_demo_mode', 'yes' );
		$this->assertTrue( $this->service->is_demo_mode() );
	}

	public function test_is_demo_mode_false_when_configured() {
		update_option( 'metricpulse_demo_mode', 'no' );
		update_option( 'metricpulse_property_id', '123456789' );
		...
		update_option( 'metricpulse_credentials', $encrypted );
	...
	public function test_connection_state_misconfigured_when_demo_off_and_no_credentials() {
		update_option( 'metricpulse_demo_mode', 'no' );
		delete_option( 'metricpulse_property_id' );
		delete_option( 'metricpulse_credentials' );
	...
	public function test_remote_ga_api_integration() {
		...
		$json_config = getenv( 'METRICPULSE_GA_JSON' );
		if ( ! $json_config && defined( 'METRICPULSE_GA_JSON' ) ) {
			$json_config = METRICPULSE_GA_JSON;
		}

		if ( ! $json_config ) {
			$this->markTestSkipped( 'METRICPULSE_GA_JSON not defined. Skipping live Google Analytics API integration test.' );
		}

		// Configure live service credentials from constant/env
		if ( ! defined( 'METRICPULSE_GA_JSON' ) ) {
			define( 'METRICPULSE_GA_JSON', $json_config );
		}
		update_option( 'metricpulse_property_id', getenv( 'METRICPULSE_GA_PROPERTY_ID' ) ?: '123456789' );
